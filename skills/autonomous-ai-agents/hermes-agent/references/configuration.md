...
### Telegram reply anchors

Telegram defaults to `reply_to_mode: first`, which makes the first response chunk quote/reply-anchor the triggering user message. Set `telegram.reply_to_mode` to the **string** `off` to disable this. The gateway must be restarted for the adapter to reload it.

Pitfall: in versions where `hermes config set telegram.reply_to_mode off` coerces `off` to YAML boolean `false`, the adapter falls back to `first`; verify the stored value is a string. This is a CLI schema-recognition bug, not valid final configuration.
