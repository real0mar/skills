...
# Backwards-compatible default used by older callers.
...
def _requested_scopes_for_mode(scope_mode: str | None) -> list[str]:
    return READONLY_SCOPES if scope_mode in {"read-only", "readonly"} else READWRITE_SCOPES


def _payload_scope_mode(payload: dict) -> str:
    return payload.get("hermes_scope_mode") or "readwrite"


def _missing_scopes_from_payload(payload: dict, requested_scopes: list[str] | None = None) -> list[str]:
    raw = payload.get("scopes") or payload.get("scope")
    if not raw:
        return []
    granted = {s.strip() for s in (raw.split() if isinstance(raw, str) else raw) if s.strip()}
    expected = requested_scopes or _requested_scopes_for_mode(_payload_scope_mode(payload))
...
def _save_pending_auth(*, state: str, code_verifier: str, scope_mode: str):
...
def get_auth_url(scope_mode: str = "readwrite"):
    ...
    requested_scopes = _requested_scopes_for_mode(scope_mode)
    ...
    _save_pending_auth(state=state, code_verifier=flow.code_verifier, scope_mode=scope_mode)
...
def exchange_auth_code(code: str):
    ...
    scope_mode = pending_auth.get("scope_mode", "readwrite")
    requested_scopes = _requested_scopes_for_mode(scope_mode)
    ...
    token_payload["hermes_scope_mode"] = scope_mode
...
def main():
    ...
    parser.add_argument(
        "--scope-mode",
        choices=["read-only", "readwrite"],
        default="readwrite",
        help="OAuth scope set to request when generating an auth URL (default: readwrite)",
    )
