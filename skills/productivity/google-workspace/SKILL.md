...
```bash
# Full Workspace read/write scopes (default)
$GSETUP --auth-url

# Full Workspace read-only scopes
$GSETUP --auth-url --scope-mode read-only
```

Note: the current bundled setup script supports full Workspace scope sets only. Use `--scope-mode read-only` when the user wants read-only Gmail/Calendar/Drive/Contacts/Sheets/Docs access.
