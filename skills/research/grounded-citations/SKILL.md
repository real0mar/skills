...
Ledger location: `$HOME/.local/share/agent-skills/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `CITATION_LEDGER`.

## How to Run

```bash
S=~/.local/share/agent-skills/skills/research/grounded-citations/scripts/sources.py
